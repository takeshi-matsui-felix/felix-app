...
temp_dir = os.path.join(tempfile.gettempdir(), "fast_camera_comp_final_v5")
os.makedirs(temp_dir, exist_ok=True)
with open(os.path.join(temp_dir, "index.html"), "w", encoding="utf-8") as f:
    f.write(CLIENT_COMPRESS_HTML)

_client_compress_func = components.declare_component("fast_camera_final_v5", path=temp_dir)
...
def main():
    ...
    # ----------------------------------------
        ...
        if prop_val and type_val:
            ...
            # ★ 確実に順序を守り、ここで先にタイトル文字を合成します（UnboundLocalErrorを完全排除）
