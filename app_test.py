...
INSP_OPTS = ["-- 選択 --", "配筋検査","躯体検査","断াস্থ検査","中間検査","社内検査(設計)","社内検査(建設)","社内検査(マーケ)","社内検査(不動産)"]
    ...
    # ----------------------------------------
        ...
        if not st.session_state.current_box:
            ...
            if not st.session_state.issue_saved:
                ...
                # ★★★【改修ポイント】現場撮影(爆速) と アルバム選択のハイブリッド ★★★
                st.markdown("##### 写真の追加")
                photo_cam = st.camera_input("📸 現場で撮影（推奨: サクサク保存できます）")
                photo_file = st.file_uploader("📂 またはアルバムから選択", type=['jpg','png','jpeg'])
                
                # どちらかが選択されていれば、それを採用する
                photo = photo_cam if photo_cam else photo_file
                ...
                for w_idx, (w_name, w_recs) in enumerate(w_groups.items()):
                    ...
                    for r_idx, r in enumerate(w_recs):
                        ...
                        with c2:
                            ...
                            # ★★★【改修ポイント】是正写真もカメラとアルバムのハイブリッド ★★★
                            # カメラが複数ズラッと並ぶと邪魔なので、折りたたみ式にしています
                            with st.expander("📸 カメラを起動して撮影"):
                                up_cam = st.camera_input("是正写真", key=f"cam_{rec_id}")
                            up_file = st.file_uploader("📂 またはアルバムから選択", key=f"up_{rec_id}", type=['jpg','png','jpeg'], label_visibility="collapsed")
                            
                            up = up_cam if up_cam else up_file
