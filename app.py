...
def db_delete_inspection_data(inspection_id): requests.delete(f"{SUPABASE_URL}/rest/v1/inspection_records?inspection_id=eq.{inspection_id}", headers=HEADERS)
...
def process_photo(upload_file):
    ...
    except: return f"data:image/jpeg;base64,{base64.b64encode(upload_file.getvalue()).decode('utf-8')}"
...
<style>
    /* ボタン・ピルの装飾 */
    div.stButton > button { border-radius: 6px; height: 50px; font-weight: bold; width: 100%; }
    footer {visibility: hidden;}
    [data-testid="stStatusWidget"] { position: fixed !important; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 99999; }
    .record-box { border-bottom: 2px solid #EEEEEE; padding-bottom: 20px; margin-bottom: 20px; }
    @media print { section[data-testid="stSidebar"], header, .stButton, .admin-delete-box, .stCheckbox, [data-testid="stFileUploadDropzone"] { display: none !important; } }
...
def main():
        ...
        with t1:
            ...
            if st.button("管理者ログイン"):
                if pwd == ADMIN_PASSWORD:
                    st.session_state.role = "admin"
                    st.query_params.auth = ADMIN_PASSWORD # パラメータ保持
                    st.rerun()
                else: st.error("Pass Error")
        with t2:
    ...
    # ログアウト処理
    if st.sidebar.button("ログアウト (全認証解除)"):
        st.session_state.clear()
        st.query_params.clear()
        st.rerun()

    menu_opts = ["物件登録（管理者）", "検査実施（管理者）", "是正実施（協力業者）", "是正確認（管理者）", "完了分一覧（共通）"] if st.session_state.role == "admin" else ["受領・是正実施（協力業者）", "完了分一覧（共通）"]
    selected_menu = st.sidebar.radio("MENU", menu_opts)

    # 1. 物件登録
    if selected_menu == "物件登録（管理者）":
        st.header("物件登録")
        n = st.text_input("新規物件名")
        if st.button("登録"):
            if n: db_post("properties", {"property_id": str(uuid.uuid4()), "property_name": n}); st.success("OK")
        ps = db_get("properties", "select=*")
        for p in ps:
            pid, pname = p.get('property_id'), p.get('property_name')
            c1, c2 = st.columns([7, 3])
            c1.write(f"🏢 {pname}")
            if c2.button("削除", key=pid):
                db_delete_property(pid); st.rerun()

    # 2. 検査実施 (Pills/Radio化でキーボード回避)
    elif selected_menu == "検査実施（管理者）":
        st.header("検査実施")
        ps = db_get("properties", "select=*")
        p_opts = [{"id": None, "name": "-- 物件選択 --"}] + [{"id": x.get('property_id'), "name": x.get('property_name')} for x in ps]
        target = st.selectbox("物件を選択", p_opts, format_func=lambda x: x['name'])
        
        ins_type = st.selectbox("検査種類", INSP_OPTS)
        
        if target['id'] and ins_type != "-- 選択 --":
            st.markdown("---")
            # 階層と部位
            floor = st.pills("階層を選択", FLOOR_OPTS[1:], key="f_pill")
            
            a_opts = AREA_OPTS_SHANAI[1:] if ins_type in SHANAI_KENSA_TYPES else AREA_OPTS_STANDARD[1:]
            area = st.pills("部位を選択", a_opts, key="a_pill")
            
            if area:
                # 分類A
                cat_dict = ISSUE_TEMPLATES.get(ins_type, {}) if ins_type in ["配筋検査", "躯体検査", "中間検査"] else ISSUE_TEMPLATES.get("社内検査(設計)", {}).get(area, {})
                cat_a = st.pills("分類Aを選択", list(cat_dict.keys()), key="cat_pill")
                
                if cat_a:
                    # 指摘D (Radioはタップするだけ)
                    d_list = cat_dict.get(cat_a, [])
                    sel_d = st.radio("よくある指摘事項D", d_list, index=None)
                    
                    st.markdown("##### 詳細・場所の追記（⚠️ここだけキーボードが出ます）")
                    desc = st.text_area("詳細を入力", label_visibility="collapsed")
                    
                    w_opts = WORK_OPTS_SHANAI if ins_type in SHANAI_KENSA_TYPES else WORK_OPTS_STANDARD
                    work = st.pills("担当工種を選択", w_opts[1:], key="w_pill")
                    
                    photo = st.file_uploader("写真を撮影", type=['jpg','png','jpeg'])
                    if photo: st.image(photo)
                    
                    if st.button("この指摘を保存"):
                        if work and (sel_d or desc) and photo:
                            final_d = (sel_d if sel_d else "") + (("：" + desc) if desc else "")
                            # 検査自体がなければ作成
                            ins_id = str(uuid.uuid4())
                            db_post("inspections", {"inspection_id": ins_id, "property_id": target['id'], "property_name": target['name'], "inspection_type": ins_type, "inspection_date": str(datetime.date.today()), "inspector": "管理者"})
                            db_post("inspection_records", {"record_id": str(uuid.uuid4()), "inspection_id": ins_id, "property_id": target['id'], "floor_level": floor, "area": area, "work_type": work, "issue_detail": final_d, "issue_photo_url": process_photo(photo), "progress_status": "是正待ち"})
                            st.success("保存完了！続けて入力できます。")
                        else: st.error("写真と工種は必須です")

    # 3. 是正実施
    elif selected_menu in ["是正実施（協力業者）", "受領・是正実施（協力業者）"]:
        st.header("是正実施")
        recs = db_get("inspection_records", "progress_status=eq.是正待ち")
        ins_all = db_get("inspections")
        imap = {i['inspection_id']: i for i in ins_all}
        
        for r in recs:
            ins = imap.get(r['inspection_id'])
            if not ins: continue
            with st.container(border=True):
                st.write(f"🏢 {ins['property_name']} / {r['floor_level']} {r['area']}")
                ...
                up = c2.file_uploader("是正写真を撮影", key=r['record_id'], type=['jpg','png','jpeg'])
                if up:
                    if st.button("報告する", key=f"btn_{r['record_id']}"):
                        db_patch("inspection_records", r['record_id'], {"progress_status": "是正確認中", "fix_photo_url": process_photo(up)})
                        st.rerun()

    # 4. 是正確認
    elif selected_menu == "是正確認（管理者）":
        st.header("是正確認")
        recs = db_get("inspection_records", "progress_status=eq.是正確認中")
        ins_all = db_get("inspections")
        imap = {i['inspection_id']: i for i in ins_all}
        for r in recs:
            ins = imap.get(r['inspection_id'])
            ...
            with st.container(border=True):
                st.write(f"🏢 {ins['property_name']} / {r['floor_level']} {r['area']}")
                st.write(f"📢 内容: {r['issue_detail']}")
                c1, c2 = st.columns(2)
                c1.image(r['issue_photo_url'], caption="Before")
                c2.image(r['fix_photo_url'], caption="After")
                ca, cb = st.columns(2)
                if ca.button("✅ 承認", key=f"ok_{r['record_id']}"):
                    db_patch("inspection_records", r['record_id'], {"progress_status": "完了"}); st.rerun()
                reason = cb.text_input("否認理由", key=f"reason_{r['record_id']}")
                if cb.button("❌ 否認", key=f"ng_{r['record_id']}"):
                    db_patch("inspection_records", r['record_id'], {"progress_status": "是正待ち", "reject_reason": reason}); st.rerun()

    # 5. 完了分一覧 (削除ボタン入り)
    elif selected_menu == "完了分一覧（共通）":
        st.header("完了報告書")
        recs = db_get("inspection_records", "progress_status=eq.完了")
        ins_all = db_get("inspections")
        imap = {i['inspection_id']: i for i in ins_all}
        ...
        for r in recs:
            ...
            if t not in tree[p]: tree[p][t] = ins['inspection_id']
            
        p_sel = st.selectbox("物件", ["-- 選択 --"] + list(tree.keys()))
        if p_sel != "-- 選択 --":
            t_sel = st.selectbox("検査", ["-- 選択 --"] + list(tree[p_sel].keys()))
            if t_sel != "-- 選択 --":
                target_iid = tree[p_sel][t_sel]
                target_recs = [r for r in recs if r['inspection_id'] == target_iid]
                
                # 🚨 削除エリア (管理者のみ)
                if st.session_state.role == "admin":
                    st.markdown(f"""<div class="admin-delete-box" style="background:#FFF0F0; padding:20px; border:2px solid #FF4B4B; border-radius:10px; margin-bottom:20px;">
                        <h3 style="color:#FF4B4B; margin-top:0;">📋 完了物件の保存及び削除</h3>
                        <p style="font-size:14px;">PDF保存が完了したら、容量確保のため削除してください。</p>
                    </div>""", unsafe_allow_html=True)
                    dp = st.text_input("削除パスワード (5963)", type="password")
                    if st.button("🚨 この検査データを完全に削除する"):
                        if dp == DELETE_PASSWORD: db_delete_inspection_data(target_iid); st.success("削除完了"); st.rerun()
                        else: st.error("Pass Error")
                
                # 報告書表示
                st.markdown(f"## {p_sel} / {t_sel}")
                html = '<table style="width:100%; border-collapse:collapse; border:1px solid #000;">'
                html += '<tr style="background:#eee;"><th>No</th><th>場所</th><th>Before</th><th>詳細</th><th>After</th></tr>'
                for idx, r in enumerate(target_recs):
                    html += f'<tr><td style="border:1px solid #000; text-align:center;">{idx+1}</td>'
                    html += f'<td style="border:1px solid #000; text-align:center;">{r["floor_level"]}<br>{r["area"]}</td>'
                    html += f'<td style="border:1px solid #000; text-align:center;"><img src="{r["issue_photo_url"]}" width="150"></td>'
                    html += f'<td style="border:1px solid #000; padding:5px;">{r["issue_detail"]}</td>'
                    html += f'<td style="border:1px solid #000; text-align:center;"><img src="{r["fix_photo_url"]}" width="150"></td></tr>'
                html += '</table>'
