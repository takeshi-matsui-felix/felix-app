...
# ✨【修正ポイント】印刷時に「物件名以下」しか見えないようにする強力なフィルター
st.markdown("""
<style>
    div.stButton > button {
    ...
    @media print {
        ...
        /* Streamlitのデフォルト見出し(「完了分一覧」など)を全て非表示 */
        h1, h2, h3, h4, h5, h6 {
            display: none !important;
        }
        /* ボタン類、パスワード入力枠、削除エリア、線を非表示 */
        .stButton, 
...
def main():
    ...
    # ----------------------------------------
        ...
        if not tree and not (prop_val and type_val):
            st.info("対象の項目はありません。")
            
        for p_idx, (p_name, types) in enumerate(tree.items()):
            # ドリルダウン中（報告書表示中）は他の物件リストを隠す
            if prop_val and type_val:
                break
            with st.expander(p_name):
        ...
        if prop_val and type_val:
            ...
            if t_ids:
                ...
                if status == "完了":
                    ...
                    # ✨【修正ポイント】報告書のタイトル部分をシステム見出しではなく「div」で作ることで、印刷時に他の見出しと切り離して綺麗に残します
                    html = f"""<div id="print-report-wrapper" style="background:white; padding:0; font-family:sans-serif;">
