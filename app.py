...
def process_photo(upload_file):
    ...
    try:
        ...
        # 高速化のため、リサイズ幅と圧縮率を最適化（画質は保ちつつ通信を軽く）
...
def main():
    ...
    if st.session_state.active_menu == "物件登録（管理者）":
        ...
        for p in props:
            ...
            if c2.button("削除", key=f"d_{prop_id}"): db_delete_property(prop_id); st.rerun()
