            ...
            if not isinstance(current_records, list) or len(current_records) == 0:
                ...
                for r in reversed(current_records):
                    ...
                    with st.expander(f"{head_text} {r_detail[:20]}..."):
                        ...
                        new_w = st.selectbox("工種を変更", work_opts, index=work_opts.index(r.get('work_type')) if r.get('work_type') in work_opts else 0, key=f"w_{r_id}")
                        new_detail = st.text_area("指摘内容を変更", value=r_detail, key=f"d_{r_id}")
                        new_photo = st.file_uploader("写真を差し替え (※変更なしなら空のまま)", type=['jpg','png','jpeg'], key=f"p_{r_id}")
                        
                        col_u, col_d = st.columns(2)
                        if col_u.button("💾 この内容で更新", key=f"upd_{r_id}"):
                        ...
                        if col_d.button("🗑️ この指摘を削除", key=f"del_rec_{r_id}"):
