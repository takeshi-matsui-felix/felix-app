...
# 🛡【絶対防御フィルター】幽霊データ（Null）を完全に弾く安全な通信関数
def db_get(table, params=""):
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            # リスト（配列）で返ってきた場合、中身が空っぽ（None）のゴミデータをすべて排除する
...
# --- すべての社内検査で「社内検査(設計)」のテンプレートを共通利用するためのリスト ---
...
def main():
    ...
    if st.session_state.active_menu == "物件登録（管理者）":
        ...
        for p in props:
            ...
            if c2.button("✕", key=f"d_{prop_id}"): db_delete_property(prop_id); st.rerun()
        ...
        if st.session_state.current_box is None:
            ...
            if st.button("検査スタート"):
                ...
                if prop_name != "-- 選択 --" and ins_type != "-- 選択 --":
                    nid = str(uuid.uuid4())
                    db_post("inspections", {"inspection_id": nid, "property_id": prop_id, "property_name": prop_name, "inspection_type": ins_type, "inspection_date": str(datetime.date.today()), "inspector": "管理者"})
    ...
    # 🛡【絶対防御】是正実施 (幽霊データを弾く)
        ...
        # Noneや空のデータを完全ガード
        if not isinstance(all_ins, list): all_ins = []
        if not isinstance(all_recs, list): all_recs = []
        
        ins_map = {}
        for i in all_ins:
            if isinstance(i, dict) and i.get('inspection_id'):
                ins_map[i['inspection_id']] = i
        ...
        if st.session_state.drill_target:
            ...
            t_ids = [i.get('inspection_id') for i in all_ins if isinstance(i, dict) and i.get('property_name') == sel['prop'] and i.get('inspection_type') == sel['type']]
            valid_t_ids = [str(tid) for tid in t_ids if tid]
            
            if valid_t_ids:
                recs = db_get("inspection_records", f"inspection_id=in.({','.join(valid_t_ids)})&progress_status=eq.是正待ち")
                w_groups = {}
                for r in recs:
                    if not isinstance(r, dict): continue
    ...
    # 🛡【絶対防御】是正確認 / 完了分一覧 (幽霊データを弾く)
        ...
        if st.session_state.drill_target:
            ...
            if valid_t_ids:
                recs = db_get("inspection_records", f"inspection_id=in.({','.join(valid_t_ids)})&progress_status=eq.{status}")
                    ...
                    for w_name, w_recs in w_groups.items():
                        ...
                        for r in w_recs:
                            ...
                            with st.expander(title):
                                ...
                                if st.button("承認（完了へ）", key=f"ok_{rec_id}"): 
                                    db_patch("inspection_records", rec_id, {"progress_status": "完了"})
                                    st.rerun()
                                reason = st.text_input("否認理由", key=f"re_{rec_id}")
                                if st.button("否認（差し戻し）", key=f"ng_{rec_id}"): 
