...
def main():
    ...
    # 3. 是正実施 (KeyError防止の安全装置付き)
        ...
        for r in all_recs:
            ...
            if ins:
                p, t = ins.get('property_name', '不明'), ins.get('inspection_type', '不明')
                ...
                for w_name, w_recs in w_groups.items():
                    ...
                    for r in w_recs:
                        ...
                        with st.expander(title):
                            ...
                            if r.get('issue_photo_url'): st.image(r.get('issue_photo_url'), caption="指摘時")
                            
                            up = st.file_uploader("是正写真をアップロード", key=f"up_{r.get('record_id')}", type=['jpg','png','jpeg'])
                            if st.button("完了報告", key=f"s_{r.get('record_id')}"):
                                if up: 
                                    db_patch("inspection_records", r.get('record_id'), {"progress_status": "是正確認中", "fix_photo_url": process_photo(up)})
                                    st.rerun()
                                else: st.error("写真が必要です")
            else:
                st.info("対象の項目がありません。")

    # 4. 是正確認 / 5. 完了分一覧 (KeyError防止の安全装置付き)
                ...
                if status == "完了":
                    ...
                    for w_name, w_recs in w_groups.items():
                        ...
                        for idx, r in enumerate(w_recs):
                            ...
                            floor = r.get('floor_level', '')
                            area = r.get('area', '')
                            loc_text = f"{floor}<br>{area}"
                            if floor == "一式": loc_text = "-"
                    ...
                    for w_name, w_recs in w_groups.items():
                        ...
                        for r in w_recs:
                            ...
                            with st.expander(title):
                                ...
                                if st.button("承認（完了へ）", key=f"ok_{r.get('record_id')}"): 
                                    db_patch("inspection_records", r.get('record_id'), {"progress_status": "完了"}); st.rerun()
                                reason = st.text_input("否認理由", key=f"re_{r.get('record_id')}")
                                if st.button("否認（差し戻し）", key=f"ng_{r.get('record_id')}"): 
                                    db_patch("inspection_records", r.get('record_id'), {"progress_status": "是正待ち", "reject_reason": reason}); st.rerun()
