...
def main():
    ...
    # 3. 是正実施 (KeyError完全防止・安全アクセス版)
        ...
        ins_map = {i.get('inspection_id'): i for i in all_ins}
        tree = {}
        for r in all_recs:
        ...
        if st.session_state.drill_target:
            ...
            t_ids = [i.get('inspection_id') for i in db_get("inspections", f"property_name=eq.{sel['prop']}&inspection_type=eq.{sel['type']}")]
    ...
    # 4. 是正確認 / 5. 完了分一覧 (KeyError完全防止・安全アクセス版)
