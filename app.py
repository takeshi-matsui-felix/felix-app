    ...
    # 2. 検査実施
        ...
        if st.session_state.current_box is None:
            ...
            if not st.session_state.issue_saved:
                ...
                def on_temp_change():
                    if st.session_state.sel_temp != "-- 定型文から選ぶ --":
                        st.session_state.issue_text = st.session_state.sel_temp

                st.selectbox("よくある指摘事項", temp_list, key="sel_temp", on_change=on_temp_change)
                
                if "issue_text" not in st.session_state: st.session_state.issue_text = ""
                desc = st.text_area("指摘内容 (手入力・追記可能)", key="issue_text")
                
                photo = st.file_uploader("撮影", type=['jpg','png','jpeg'])
                if photo: st.image(photo)
                
                if st.button("この内容で保存"):
                    if w != "-- 選択 --" and desc.strip() != "" and photo is not None:
                        ...
                        st.session_state.issue_saved = True; st.session_state.issue_text = ""; st.rerun()
                    else: st.error("工種・内容・写真は必須です")
                if st.button("終了"): st.session_state.current_box = None; st.rerun()
            else:
                st.success("保存完了"); 
                if st.button("続けて次を登録"): st.session_state.issue_saved = False; st.session_state.issue_text = ""; st.rerun()
