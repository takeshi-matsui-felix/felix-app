...
def main():
    ...
    if st.session_state.active_menu == "物件登録（管理者）":
        ...
        for idx, p in enumerate(props):
            prop_id = p.get('property_id') or f"unknown_{idx}"
    ...
    # 2. 検査実施
        ...
        if not st.session_state.current_box:
            ...
            # 🛡【B案】直近履歴の修正・削除機能 (100%エラー回避キー設定)
            st.markdown("---")
            st.markdown("### 🔽 今登録した指摘一覧")
            current_records = db_get("inspection_records", f"inspection_id=eq.{c_id}")
            if not isinstance(current_records, list) or len(current_records) == 0:
                st.info("まだ登録された指摘はありません。")
            else:
                for idx, r in enumerate(reversed(current_records)):
                    if not isinstance(r, dict): continue
                    r_id = r.get('record_id') or str(uuid.uuid4())
                    key_suffix = f"{r_id}_{idx}"
                    
                    r_floor = r.get('floor_level', '')
                    r_area = r.get('area', '')
                    r_detail = r.get('issue_detail', '')
                    head_text = f"【{r_floor} {r_area}】" if r_floor != "一式" else ""
                    
                    with st.expander(f"{head_text} {r_detail[:20]}..."):
                        if r.get('issue_photo_url'): st.image(r.get('issue_photo_url'), width=200)
                        
                        new_w = st.selectbox("工種を変更", work_opts, index=work_opts.index(r.get('work_type')) if r.get('work_type') in work_opts else 0, key=f"w_{key_suffix}")
                        new_detail = st.text_area("指摘内容を変更", value=r_detail, key=f"d_{key_suffix}")
                        new_photo = st.file_uploader("写真を差し替え (※変更なしなら空のまま)", type=['jpg','png','jpeg'], key=f"p_{key_suffix}")
                        
                        col_u, col_d = st.columns(2)
                        if col_u.button("💾 この内容で更新", key=f"upd_{key_suffix}"):
                            update_data = {"work_type": new_w, "issue_detail": new_detail}
                            if new_photo:
                                update_data["issue_photo_url"] = process_photo(new_photo)
                            db_patch("inspection_records", r_id, update_data)
                            st.success("更新しました！")
                            st.rerun()
                            
                        if col_d.button("🗑️ この指摘を削除", key=f"del_rec_{key_suffix}"):
                            db_delete_record(r_id)
                            st.rerun()
            ...
            if t_ids:
                ...
                for w_idx, (w_name, w_recs) in enumerate(w_groups.items()):
                    ...
                    for r_idx, r in enumerate(w_recs):
                        ...
                        rec_id = r.get('record_id') or str(uuid.uuid4())
                        key_suffix = f"{rec_id}_{w_idx}_{r_idx}"
