...
AREA_OPTS_SHANAI = ["-- 選択 --", "玄関", "トイレ", "キッチン", "LDK", "バル শুকニー", "洋室", "洗面室", "UB", "廊下・階段・ENT", "外部", "その他"]
...
def main():
    ...
    if st.session_state.active_menu == "物件登録（管理者）":
        ...
        for idx, p in enumerate(props):
            ...
            safe_prop_id = prop_id if prop_id else f"empty_prop_{idx}"
            prop_name = p.get('property_name', '不明')
            
            c1, c2 = st.columns([7, 3])
            if c1.button(f"{prop_name} 検査へ", key=f"p_{safe_prop_id}"): jump_to_menu("検査実施（管理者）", safe_prop_id)
            
            if c2.button("削除", key=f"d_{safe_prop_id}"):
                st.session_state.delete_target = safe_prop_id
                st.rerun()
                
            if st.session_state.delete_target == safe_prop_id:
                st.warning(f"⚠️ 本当に「{prop_name}」を削除しますか？紐づくすべてのデータが消えます。")
                del_pw = st.text_input("削除用パスワードを入力", type="password", key=f"pw_{safe_prop_id}", placeholder="2011")
                col_y, col_n = st.columns(2)
                if col_y.button("Yes (削除実行)", key=f"yes_{safe_prop_id}"):
                    if del_pw == "2011":
                        db_delete_property(safe_prop_id)
                        st.session_state.delete_target = None
                        st.session_state.current_box = None
                        st.rerun()
                    else:
                        st.error("パスワードが違います")
                if col_n.button("No (キャンセル)", key=f"no_{safe_prop_id}"):
                    st.session_state.delete_target = None
                    st.rerun()
                st.markdown("---")
    ...
    # 2. 検査実施
        ...
        if not st.session_state.current_box:
            ...
            if not st.session_state.issue_saved:
                ...
                # 📍【工種選択の配置】詳細の直後、写真の前に移動
                w = st.selectbox("工種を選択", work_opts)
                
                photo = st.file_uploader("撮影", type=['jpg','png','jpeg'])
                if photo is not None: st.image(photo, use_container_width=True)
    ...
    # 3. 是正実施
        ...
        if prop_val and type_val:
            ...
            if t_ids:
                ...
                # 編集画面用に、その検査種別に応じた工種リストを用意
                if type_val in SHANAI_KENSA_TYPES: edit_w_opts = WORK_OPTS_SHANAI
                elif type_val == "躯体検査": edit_w_opts = WORK_OPTS_KUTAI
                elif type_val == "配筋検査": edit_w_opts = WORK_OPTS_HAIKIN
                elif type_val == "中間検査": edit_w_opts = WORK_OPTS_CHUKAN
                ...
                for w_idx, (w_name, w_recs) in enumerate(w_groups.items()):
                    ...
                    for r_idx, r in enumerate(w_recs):
                        ...
                        # 📍【超重要】エラー回避のため固定のキーを生成
                        rec_id = r.get('record_id')
                        safe_rec_id = rec_id if rec_id else f"no_id_{w_idx}_{r_idx}"
                        key_suffix = f"{safe_rec_id}"
                        ...
                        with st.expander(title):
                            ...
                            if st.session_state.role == "admin":
                                if st.checkbox("⚙️ 是正内容編集 (管理者専用)", key=f"edit_chk_{key_suffix}"):
                                    st.markdown("#### 📝 データ編集")
                                    new_detail = st.text_area("指摘内容を変更", value=detail, key=f"edit_d_{key_suffix}")
                                    
                                    current_w = r.get('work_type', '')
                                    idx_w = edit_w_opts.index(current_w) if current_w in edit_w_opts else 0
                                    new_w = st.selectbox("工種を変更", edit_w_opts, index=idx_w, key=f"edit_w_{key_suffix}")
                                    
                                    new_photo = st.file_uploader("写真を差し替え (変更なしなら空)", type=['jpg','png','jpeg'], key=f"edit_p_{key_suffix}")
                                    # 📍【修正】管理者の差し替え写真プレビューを完璧に表示
                                    if new_photo is not None:
                                        st.image(new_photo, caption="差し替えプレビュー", use_container_width=True)
                                    
                                    col_u, col_d = st.columns(2)
                                    if col_u.button("💾 更新を保存", key=f"edit_save_{key_suffix}"):
                                        ...
                                        st.success("更新しました！")
                                        st.rerun()
                                    if col_d.button("🗑️ この指摘を削除", key=f"edit_del_{key_suffix}"):
                                        db_delete_record(rec_id)
                                        st.rerun()
                                    st.markdown("---")

                            # --- 協力業者用（または管理者代理用）の是正報告 ---
                            up = st.file_uploader("是正写真をアップロード", key=f"up_{key_suffix}", type=['jpg','png','jpeg'])
                            if up is not None: st.image(up, caption="アップロード画像プレビュー", use_container_width=True)
                            
                            if st.button("完了報告", key=f"s_{key_suffix}"):
                                if up is not None: 
                ...
                if status == "完了":
                    ...
                    for w_idx, (w_name, w_recs) in enumerate(w_groups.items()):
                        ...
                        for r_idx, r in enumerate(w_recs):
                            ...
                            rec_id = r.get('record_id')
                            safe_rec_id = rec_id if rec_id else f"no_id_c_{w_idx}_{r_idx}"
                            key_suffix = f"{safe_rec_id}"
                            ...
                            ca, cb = st.columns(2)
                            if ca.button("✅ 承認（完了へ）", key=f"ok_{key_suffix}"): 
                                db_patch("inspection_records", rec_id, {"progress_status": "完了"})
                                st.rerun()
                            
                            reason = cb.text_input("否認理由を入力", key=f"re_{key_suffix}", label_visibility="collapsed", placeholder="否認理由があれば入力")
                            if cb.button("❌ 否認（差し戻し）", key=f"ng_{key_suffix}"): 
                                db_patch("inspection_records", rec_id, {"progress_status": "是正待ち", "reject_reason": reason})
                                st.rerun()
                            
                            st.markdown("---") 

# 🛡 万が一、予期せぬエラーが起きた場合の最終セーフティネット
if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        st.error(f"システムエラーが発生しました。過去の不良データが残っている可能性があります。")
        st.info("下の【システム復旧】ボタンを押して画面をリセットしてください。")
        if st.button("システム復旧"):
