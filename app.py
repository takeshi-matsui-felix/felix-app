...
# ==========================================
# 2. UI設定 (アイコン設定・スマホ対応完全版)
# ==========================================
st.set_page_config(page_title="Felix検査App", page_icon="icon.png", layout="wide")

# 📱 スマホ（iPhone/Android）のホーム画面用アイコン設定
# 画像がない場合でも絶対にエラーで落ちないように安全装置(try-except)を追加
try:
    with open("icon.png", "rb") as f:
        img_base64 = base64.b64encode(f.read()).decode()
    st.markdown(f'''
        <link rel="apple-touch-icon" href="data:image/png;base64,{img_base64}">
        <link rel="shortcut icon" sizes="192x192" href="data:image/png;base64,{img_base64}">
        <link rel="icon" sizes="192x192" href="data:image/png;base64,{img_base64}">
...
def main():
    ...
    if st.session_state.active_menu == "物件登録（管理者）":
        ...
        for idx, p in enumerate(props):
            ...
            if not prop_id: continue # 壊れたデータを無視
